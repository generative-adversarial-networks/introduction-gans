...
def get_data_samples(list_of_centers, sigma, batch_size):
    # chunk_size = int(batch_size/len(list_of_centers))
    # list_of_tensors = []
    # for x,y in list_of_centers:
    #     list_of_tensors.append(get_data_points(x, y, sigma, chunk_size))
    # list_of_tensors = torch.cat(list_of_tensors, dim=0)
    # points = list_of_tensors.detach().numpy()
    #return torch.from_numpy(points[np.random.choice(points.shape[0], batch_size), :]).float()
